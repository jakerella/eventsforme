...
Ext.define('Ext.io.Developer', {
    ...
    send: function(options,callback,scope) {
        ...
        if(Ext.cf.util.ParamValidator.validateOptionsCallbackScope([{ name: "message", type: 'string|object' }],arguments, "Ext.io.Developer", "send")) {
            self.getChannelKey(function(channel, err) {
                if (channel) {
                    Ext.io.Io.getMessagingProxy(function(messaging){
                        messaging.pubsub.publish(self.developerChannelName, channel, options.message, callback, scope);
                    },self);
                } else {
